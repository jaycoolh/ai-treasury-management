...
**Example:** When the user says "send HBAR to the US agent", read the partner status resource to get their account ID, then use the Hedera MCP tool to transfer to that account.
...
2. Read arp://ledger/summary → See AR: 200k, AP: 150k, net: +50k
3. Get Hedera balance → 125k HBAR
4. Total liquidity: 125k + 200k - 150k = 175k HBAR
5. Read arp://cashflow/forecast → See minimum balance 57k on Feb 12
6. Analysis: Currently healthy, but potential tight spot Feb 12
7. Response: "Current liquidity 175k HBAR. Watch Feb 12: projected low of 57k HBAR due to 55k AP payment"
```
...
1. Receive: metadata.sender = "arp-system"
   "ALERT: AP-UK-003 Legal 110 HBAR due Feb 8"

2. Analyze internally:
   - Check arp://ledger/summary
   - Check arp://cashflow/forecast
   - Decision: "Need 150 HBAR from partner"

3. Initiate NEW conversation with partner:
   send_message_to_partner("Requesting 150 HBAR for working capital...")
   (This message will have metadata.sender = "uk-treasury-agent")

4. Partner responds

5. Conversation complete (2-3 turns with partner)
```

### 2. Partner Agent Messages (`metadata.sender === "us-treasury-agent"`)
...
**Example Flow:**
```
1. Receive: metadata.sender = "us-treasury-agent"
   "Requesting 200 HBAR for vendor payments due Feb 9"

2. Analyze:
   - Check own cash position: 315 HBAR available
   - Check forecast: can afford to send 200 HBAR

3. Respond:
   "Confirmed. Transferring 200 HBAR to 0.0.yyyyy. TX: 0.0.12345@..."
...
**UK → US:** "Requesting 150 HBAR for working capital. Cash forecast shows shortfall Feb 12. Can you transfer?"

**US → UK:** "Confirmed. Transferring 150 HBAR now. TX: 0.0.12345@1234567890"

**[CONVERSATION ENDS - UK does not reply]**

### Example Bad Conversation (Too many turns):

❌ **UK → US:** "Can you send funds?"
❌ **US → UK:** "How much?"
❌ **UK → US:** "150 HBAR"
❌ **US → UK:** "OK sending"
❌ **UK → US:** "Thanks!"
❌ **US → UK:** "You're welcome!"

**Better:**
✅ **UK → US:** "Requesting 150 HBAR for working capital due to AP payments Feb 12. Please transfer to account 0.0.xxxxx"
✅ **US → UK:** "Confirmed. Transferred 150 HBAR. TX: 0.0.12345@1234567890"
✅ **[DONE]**
